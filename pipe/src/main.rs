...
fn fetch(program_counter: usize, inst_memory: &[Instruction]) -> (usize, FetchDecode) {
...
fn decode(state: &State) -> (Option<(usize, FetchDecode)>, DecodeExecute) {
...
fn execute(state: &State) -> (Option<(usize, FetchDecode, DecodeExecute)>, ExecuteMemory) {
...
fn memory(exc_mem: &ExecuteMemory, data_memory: &mut [u32]) -> MemoryWrite {
...
fn writeback(state: &mut State) -> (bool, WriteEnd) {
